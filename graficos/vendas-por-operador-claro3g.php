...
$conINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && operador = '".$operador ."' && (status = 'AUTORIZADA' || status = 'PÓS VENDAS') && data_autorizacao LIKE '%".$ano.$mes."%'");
...
$conCONECT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && operador = '".$operador ."' && status = 'DEVOLVIDO' && data LIKE '%".$ano.$mes."%'");
$totalCONECT = mysql_num_rows($conCONECT);
