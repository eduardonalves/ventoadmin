...
function mostrarinfo(id){
	...
	$('#infooperador .load').load("includes/infooperador.php?id="+id+"&pro=<?= $produto_id;?>&m=<?= $mes;?>&an=<?= $ano;?>", function(){
...
function carregargrafico(id){
	...
	document.getElementById('graficoop'+id).innerHTML = '<iframe src="graficos/vendas-por-operador-oivelox?o='+id+'&m=<?= $mes;?>&a=<?= $ano;?>&produto=<?= $produto_id;?>" width="620px" height="410px" frameborder="0" scrolling="no" ></iframe>';
...
<td style="font-size:14px; color:#999;">ESTAT&Iacute;STICAS (Oi)</td>

<td align="right"><img src="img/voltar.png" style="cursor:pointer" onclick="window.location = '?p=oi'" /></td>
...
<option value="06" <?  if($_GET['produto']== 06){echo "selected=selected";} ?>>Oivelox</option>

<option value="04" <?  if($_GET['produto']== 04){echo "selected=selected";} ?>>Oitv </option>
...
<td colspan="9" align="center"><b style="color:#FFF"><?= $es1;?></b></td>
...
<td bgcolor="#F9BB11" width="100px">Número de Conectadas</td>

<td bgcolor="#AD820C"> &nbsp; Percentual de Conectadas &nbsp; </td>
...
$conINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && data_instalacao LIKE '%".$ano.$mes."%' && status = 'FINALIZADA' && monitor LIKE '%".$loginMONITOR."%'");
...
$conCONECTADAS = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && status = 'CONECTADO' && data_conectada LIKE '%".$ano.$mes."%' && monitor LIKE '%".$loginMONITOR."%'");

$totalCONECTADAS = mysql_num_rows($conCONECTADAS);
...
$conNUMINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && data_instalacao = '".$ano.$mes.$n."' && status = 'FINALIZADA' && monitor LIKE '%".$loginMONITOR."%'");
...
$conNUMCONECTADAS = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && status = 'CONECTADO' && data_conectada = '".$ano.$mes.$n."' && monitor LIKE '%".$loginMONITOR."%'");

$NUMCONECTADAS = mysql_num_rows($conNUMCONECTADAS);
...
if($totalCONECTADAS > 0){ $porcentCONECTADAS = ($NUMCONECTADAS/$totalCONECTADAS)*100;}
...
<td align="left"><?= ceil($porcentCONECTADAS); ?>% <div style="width:<?= ceil(($porcentCONECTADAS)*2);?>px; background-color:#ec2626; height:3px;"></div></td>
...
<td colspan="5" align="center"><b style="color:#FFF"><?= $es3;?></b></td>
...
$conMONITORES = $conexao->query("SELECT usuarios.nome AS nomemonitor,
										...
										COUNT(if((vendas_clarotv.data_conectada LIKE '%".$ano.$mes."%' && 

												  vendas_clarotv.status = 'CONECTADO'),1,NULL)) AS vendasconectadas, 												  
										...
										WHERE vendas_clarotv.produto = '".$produto_id."' && 
											  ...
											  (vendas_clarotv.data LIKE '%".$ano.$mes."%' || 
											   ...
											   vendas_clarotv.data_instalacao LIKE '%".$ano.$mes."%' || vendas_clarotv.data_conectada LIKE '%".$ano.$mes."%') && 
...
<tr bgcolor="#999999" style="font-size:14px; color:#FFF; font-weight:bold" align="left"><td colspan="5">Monitor: <?= $MONITORES['nomemonitor'];?></td></tr>
...
<td bgcolor="#ceedfc" width="100px">Vendas Conectadas</td>
...
$conOPERADORES = $conexao->query("SELECT operadores.nome AS nomeoperador,
								  ...
								  WHERE vendas_clarotv.produto = '".$produto_id."' && 
										...
										(vendas_clarotv.data LIKE '%".$ano.$mes."%' || 
										 ...
										 vendas_clarotv.data_conectada LIKE '%".$ano.$mes."%' || vendas_clarotv.data_instalacao LIKE '%".$ano.$mes."%')
...
<td><?= ceil($OPERADORES['vendasconectadas']);?></td>
...
<td><?= $MONITORES['vendasconectadas'];?></td>
...
<td title="Data Início" onclick="window.location = '?p=estatisticas-oivelox&m=<?= $_GET['m'];?>&an=<?= $_GET['an'];?>&es=<?= $_GET['es'];?>&o=<? if($_GET['o'] == 'operadorData ASC'){ echo 'operadorData DESC'; } else { echo 'operadorData ASC'; }?>'">Data Início <? if($_GET['o'] == 'operadorData ASC'){ ?><img src="img/seta-u.png" /> <? } else if($_GET['o'] == 'operadorData DESC'){ ?> <img src="img/seta-d.png" /> <? } ?></td>



<td title="Tipo Contrato" onclick="window.location = '?p=estatisticas-oivelox&m=<?= $_GET['m'];?>&an=<?= $_GET['an'];?>&es=<?= $_GET['es'];?>&o=<? if($_GET['o'] == 'operadorContrato ASC'){ echo 'operadorContrato DESC'; } else { echo 'operadorContrato ASC'; }?>'">Tipo Contrato <? if($_GET['o'] == 'operadorContrato ASC'){ ?><img src="img/seta-u.png" /> <? } else if($_GET['o'] == 'operadorContrato DESC'){ ?> <img src="img/seta-d.png" /> <? } ?></td>



<td title="Nome do Operador" onclick="window.location = '?p=estatisticas-oivelox&m=<?= $_GET['m'];?>&an=<?= $_GET['an'];?>&es=<?= $_GET['es'];?>&o=<? if($_GET['o'] != 'operadores.nome DESC'){ echo 'operadores.nome DESC'; } else { echo 'operadores.nome ASC'; }?>'">Nome do Operador <? if($_GET['o'] == 'operadores.nome ASC'){ ?><img src="img/seta-u.png" /> <? } else if($_GET['o'] == 'operadores.nome DESC'){ ?> <img src="img/seta-d.png" /> <? } ?></td>



<td title="Vendas Feitas" onclick="window.location = '?p=estatisticas-oivelox&m=<?= $_GET['m'];?>&an=<?= $_GET['an'];?>&es=<?= $_GET['es'];?>&o=<? if($_GET['o'] == 'vendas ASC'){ echo 'vendas DESC'; } else { echo 'vendas ASC'; }?>'">Vendas Feitas <? if($_GET['o'] == 'vendas ASC'){ ?><img src="img/seta-u.png" /> <? } else if($_GET['o'] == 'vendas DESC'){ ?> <img src="img/seta-d.png" /> <? } ?></td>



<td title="Vendas Finalizadas" onclick="window.location = '?p=estatisticas-oivelox&m=<?= $_GET['m'];?>&an=<?= $_GET['an'];?>&es=<?= $_GET['es'];?>&o=<? if($_GET['o'] == 'finalizadas DESC' || $_GET['o'] == ''){ echo 'finalizadas ASC'; } else { echo 'finalizadas DESC'; }?>'">Vendas Finalizadas <? if($_GET['o'] == 'finalizadas DESC' || $_GET['o'] == ''){ ?><img src="img/seta-d.png" /> <? } else if($_GET['o'] == 'finalizadas ASC'){ ?> <img src="img/seta-u.png" /> <? } ?></td>



<td title="Vendas Conectadas" onclick="window.location = '?p=estatisticas-oivelox&m=<?= $_GET['m'];?>&an=<?= $_GET['an'];?>&es=<?= $_GET['es'];?>&o=<? if($_GET['o'] == 'conectadas ASC'){ echo 'conectadas DESC'; } else { echo 'conectadas ASC'; }?>'">Vendas Conectadas <? if($_GET['o'] == 'conectadas ASC'){ ?><img src="img/seta-u.png" /> <? } else if($_GET['o'] == 'conectadas DESC'){ ?> <img src="img/seta-d.png" /> <? } ?></td>
...
<td title="Meta de Qualidade" onclick="window.location = '?p=estatisticas-oivelox&m=<?= $_GET['m'];?>&an=<?= $_GET['an'];?>&es=<?= $_GET['es'];?>&o=<? if($_GET['o'] == 'metaqualidade ASC'){ echo 'metaqualidade DESC'; } else { echo 'metaqualidade ASC'; }?>'">Meta de Qualidade <? if($_GET['o'] == 'metaqualidade ASC'){ ?><img src="img/seta-u.png" /> <? } else if($_GET['o'] == 'metaqualidade DESC'){ ?> <img src="img/seta-d.png" /> <? } ?></td>





<td title="Aproveitamento" onclick="window.location = '?p=estatisticas-oivelox&m=<?= $_GET['m'];?>&an=<?= $_GET['an'];?>&es=<?= $_GET['es'];?>&o=<? if($_GET['o'] == 'aproveitamento ASC'){ echo 'aproveitamento DESC'; } else { echo 'aproveitamento ASC'; }?>'">Aproveitamento <? if($_GET['o'] == 'aproveitamento ASC'){ ?><img src="img/seta-u.png" /> <? } else if($_GET['o'] == 'aproveitamento DESC'){ ?> <img src="img/seta-d.png" /> <? } ?></td></tr>
...
$conOPERADORES1 = $conexao->query("SELECT *, COUNT(IF(vendas_clarotv.data LIKE '%".$ano.$mes."%', 1, NULL)) AS vendas,
									 ...
									 COUNT(IF(vendas_clarotv.status = 'FINALIZADA' && 
									 		  ...
									 		  vendas_clarotv.data_instalacao LIKE '%".$ano.$mes."%', 1, NULL)) AS finalizadas,
									 COUNT(IF(vendas_clarotv.status = 'CONECTADO' && 

									 		  vendas_clarotv.data_conectada LIKE '%".$ano.$mes."%', 1, NULL)) AS conectadas,											  
									 ...
									 (data LIKE '%".$ano.$mes."%' || data_conectada LIKE '%".$ano.$mes."%' || vendas_clarotv.data_instalacao LIKE '%".$ano.$mes."%') && 
...
$totalVendasFinalizadas = $totalVendasFinalizadas + $OPERADORES1['finalizadas'];

$totalVendasConectadas = $totalVendasConectadas + $OPERADORES1['conectadas'];
...
<td><?= $OPERADORES1['finalizadas'];?></td>

<td><?= $OPERADORES1['conectadas'];?></td>

<td><span <? if($OPERADORES1['finalizadas'] < $vendasEsperadas){ ?>style="color:#D00"<? } else { ?> style="color:#0C0"  <? } ?>> <?= $vendasEsperadas;?> </span> </td>
...
$conVENDASFEITASM = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && data LIKE '%".$ano.$mes."%' && monitor LIKE '%".$loginMONITOR."%'");

$totalVENDASFEITASM = mysql_num_rows($conVENDASFEITASM);



$conVENDASFINALIZADASSM = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && data_instalacao LIKE '%".$ano.$mes."%' && status = 'FINALIZADA' && monitor LIKE '%".$loginMONITOR."%'");

$totalVENDASFINALIZADASM = mysql_num_rows($conVENDASFINALIZADASM);

$conVENDASCONECTADASM = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && data_conectada LIKE '%".$ano.$mes."%' && status = 'CONECTADO' && monitor LIKE '%".$loginMONITOR."%'");

$totalVENDASCONECTADASM = mysql_num_rows($conVENDASCONECTADAS);


$conTotalmetaqualidade = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && data LIKE '%".$ano.$mes."%' && (status = 'DEVOLVIDO' || status = 'CANCELADO' || status = 'SEM CONTATO') && monitor LIKE '%".$loginMONITOR."%'");
...
<td><?= $totalVendasFinalizadas;?></td>

<td><?= $totalVendasConectadas;?></td>

<td><span <? if($totalVendasFinalizadas < $TotalVendasEsperadas){ ?>style="color:#D00"<? } else { ?> style="color:#0C0"  <? } ?>> <?= $TotalVendasEsperadas;?> </span> </td>
...
<td><?= ceil(($totalVENDASFINALIZADASM * 100)/$totalVENDASFEITASM).'%';?></td>
