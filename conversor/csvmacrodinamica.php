	...
	if($_POST){
		...
		if($opt=="preanalise"){
			...
			while ($arraycsv = mysql_fetch_array($resutado)) {
				...
				$mae=str_replace(" ","<sp>",$arraycsv['nome_mae']); 
				$nome=$arraycsv['nome'];
				$nascimento=$arraycsv['nascimento'];
				$nomeAux=explode(' ', $nome);
				if(isset($nomeAux[5]) && $nomeAux[5] !=""){
					$sobrenome=$nomeAux[5];
					$nome=$nomeAux[0]."<sp>".$nomeAux[1]."<sp>".$nomeAux[2]."<sp>".$nomeAux[3]."<sp>".$nomeAux[4];
				}else if(isset($nomeAux[4]) && $nomeAux[4] !=""){
					$sobrenome=$nomeAux[4];
					$nome=$nomeAux[0]."<sp>".$nomeAux[1]."<sp>".$nomeAux[2]."<sp>".$nomeAux[3];
				}else if(isset($nomeAux[3]) && $nomeAux[3] !=""){
					$sobrenome=$nomeAux[3];
					$nome=$nomeAux[0]."<sp>".$nomeAux[1]."<sp>".$nomeAux[2];
				}else if(isset($nomeAux[2]) && $nomeAux[2] !=""){
					$sobrenome=$nomeAux[2];
					$nome=$nomeAux[0]."<sp>".$nomeAux[1];
				}else if(isset($nomeAux[1]) && $nomeAux[1] !=""){
					$sobrenome=$nomeAux[1];
					$nome=$nomeAux[0];
				}
				...
				$csv .= "WAIT SECONDS=3"."\n";
				$csv .= "TAG POS=1 TYPE=INPUT:TEXT FORM=NAME:salesForm ATTR=NAME:salesTO.customer.name CONTENT=".$nome.""."\n";
				$csv .= "WAIT SECONDS=2"."\n";
				$csv .="TAG POS=1 TYPE=INPUT:TEXT FORM=NAME:salesForm ATTR=NAME:birthDateStr CONTENT=".$nascimento.""."\n";
				$csv .= "WAIT SECONDS=2"."\n";
				$csv .="TAG POS=1 TYPE=INPUT:TEXT FORM=NAME:salesForm ATTR=NAME:salesTO.customer.lastName CONTENT=".$sobrenome.""."\n";
				$csv .= "WAIT SECONDS=2"."\n";
				$csv .="TAG POS=1 TYPE=INPUT:TEXT FORM=NAME:salesForm ATTR=NAME:salesTO.customer.motherName CONTENT=".$mae.""."\n";
				$csv .= "WAIT SECONDS=2"."\n";
				$csv .= "TAG POS=1 TYPE=INPUT:SUBMIT FORM=NAME:salesForm ATTR=ID:btProceed"."\n";
				$csv .= "WAIT SECONDS=5"."\n";
				$csv .= "TAG POS=1 TYPE=INPUT:SUBMIT FORM=NAME:salesForm ATTR=ID:btProceed"."\n";
		...
		if($opt=="preanaliseExterna2"){
			...
			while ($arraycsv = mysql_fetch_array($resutado)) {
				...
				$csv .="WAIT SECONDS=2"."\n";
				$csv .="TAG POS=1 TYPE=INPUT:PASSWORD FORM=ACTION:/CookieAuth.dll?Logon ATTR=NAME:password CONTENT=Elida2014"."\n";
				$csv .= "TAG POS=1 TYPE=INPUT:SUBMIT FORM=ACTION:/CookieAuth.dll?Logon ATTR=NAME:SubmitCreds"."\n";
				$csv .= "TAB T=2"."\n";
				$csv .= "FRAME NAME=LivreFEBodyFrame"."\n";
