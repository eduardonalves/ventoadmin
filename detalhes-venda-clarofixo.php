...
function getMonitorEsns(persistEsn){
	
	var aparelho = $('#aparelho option:selected').val();
	var monitor = $('#monitor option:selected').val();
	
	persistEsn = $('#select-esn option:selected').val();

	if( aparelho == '' || monitor == ''){

		$('.line-esn').html('<select id="select-esn" name="select-esn"><option value=""></option></select>');
	
	} else {
		
		$.ajax({url:'check-esns.php?m='+monitor+'&ap='+aparelho,success:function(result){
			$("#select-esn").remove();
			$('.line-esn').append(result);
			$("#esn_nova").html($("#select-esn").html());
			//alert(result);

			$('#select-esn').val(persistEsn);
			$('#select-esn').trigger('change');
			

		}});

		$.ajax({url:'check-esns-reposicao.php?m='+monitor+'&ap='+aparelho,success:function(result){

			$("#esn_nova").html('');
			$('#esn_nova').append(result);
			//alert(result);
		...
		$('.line-esn').load('check-esns.php?m='+monitor+'&ap='+aparelho, function (){
	...
	$('#select-esn').live('change', function(){
		
		$("[name='esn']").val( $('option:selected', this).val() );

	});


	$("#aparelho, #monitor").bind("change", function(){
...
if($esn != '')
{
			if ( 
					...
					( 
						(! verificaEsn()) && ($linha['status']!='FINALIZADA') 
					) || 
					
					(
						$linha['status']=='FINALIZADA' && ($USUARIO['tipo_usuario'] != 'ADMINISTRADOR' && $USUARIO['tipo_usuario'] != 'MONITORBO')
					)
				)

	{
			...
			{
				$varerros = 'Campos obrigatórios não preenchidos ou Inválidos:\n\n';
				$varerros .= "ESN Inválida. Esta ESN não pertence ao monitor associado a esta venda.";
...
if($numchip != '')
{
			...
			if ( 
					...
					( 
						(! verificaNumChip()) && ($linha['status']!='FINALIZADA') 
					) || 
...
if($_POST['status'] == strtoupper('FINALIZADA') && ($_POST['agendaentrega'] == "00/00/0000" || $_POST['agendaentrega'] == ""))
{
	...
	if( strtoupper($_POST['status'])=='FINALIZADA' )
	{
		...
		if( ($linha['status']!='FINALIZADA') || ($linha['status']=='FINALIZADA' && $USUARIO['tipo_usuario'] != 'ADMINISTRADOR')  )
		{
			...
			{
				$excep = array("lote", "quadra", "complemento", "rg", "orgexp", "dataexp", "os", "esn", "novonumero","numchip","itelefone3","tipotel3","pontoref");
...
<script type="text/javascript">
	...
	$(window).load(function(){
		...
		$("#esn_nova").bind('change', function(){
			
			$("[name='esn_nova']").val($('option:selected', this).val());
		
		});
		
		$("#bt-reposicao").bind('click', function(){
			
			var esn = $("[name='esn_antiga']").val();
			var esnNova = $("[name='esn_nova']").val();
			
			if (esnNova == '' || typeof esnNova === 'undefined'){
			
				alert("Selecione uma nova esn para efetuar a reposição.");
			
			}else{
				
				var confirma  = confirm("Reposição da esn \"" + esn + "\" pela nova esn \"" + esnNova + "\". Você confirma?");
				
				if (confirma) {
					
					$.post("inserir-reposicao-clarofixo.php", $("form[name='frmReposicao']").serialize(), function(result){
						
						if (result == "1"){
							
							alert('Reposição efetuada com sucesso.');
							document.location = 'detalhes-venda-clarofixo.php?e=1&id=' + <?php echo $linha['id']; ?>;
...
//////////////////////////////////	
	...
	var persistAparelho = $('#aparelho option:selected').val();
	
	$('#aparelho').html('<?php echo $optAparelhos; ?>');
	$('#aparelho').val(persistAparelho);
	...
	function verificaaparelho(v){
		...
		var ano_venda = '<?php echo substr($linha['data_venda'],0,4); ?>';
		var mes_venda = '<?php echo substr($linha['data_venda'],4,2); ?>';
		var dia_venda = '<?php echo substr($linha['data_venda'],6,2); ?>';
...
////////////// PLANO PÓS ////////////////
	...
	if ( (tipo_usuario=='MONITOR' || tipo_venda=='EXTERNA') && (<?php echo date("Y"); ?>>=2014 && <?php echo date("m"); ?>>=04 ) )
	{
		document.getElementById('valoraparelho').value = '60,00';
	}

	/*
	Promoção somente para alguns parceiros de aparelho a 40 reais até 31/05/2014
	*/

	if ( ( (monitor_venda==3176) && tipo_venda=='EXTERNA') && (ano_venda==2014 && mes_venda==05 && dia_venda>=21 ) )
	{
		document.getElementById('valoraparelho').value = '40,00';
	}
...
<table border="0" width="100%" style="font-size:14px;">
	...
	<form name="frmReposicao" action="inserir-reposicao-clarofixo.php" method="POST">

		<input type="hidden" name="id_venda" value="<?php echo $linha['id']; ?>">
		<input type="hidden" name="monitor_venda" value="<?php echo $linha['monitor'];  ?>">
		<input type="hidden" name="operador_venda" value="<?php echo $linha['operador']; ?>">
		<input type="hidden" name="esn_antiga" value="<?php echo $linha['esn']; ?>">
		<input type="hidden" name="esn_nova" value="">
...
<form id="formEditar" class="formeditar" name="editar" action="" method="post">
...
<? if( 
($linha['os'] ==""  || ($USUARIO['tipo_usuario'] =='ADMINISTRADOR' || $USUARIO['tipo_usuario'] =='AUDITOR' || $USUARIO['tipo_usuario'] =='MONITORBO' || $USUARIO['id']==3129) )
 && ($editar == '1' || $editar_instalacao == '1') && ( (($USUARIO['tipo_usuario']!="MONITOR" && $USUARIO['acesso_usuario']!="EXTERNO") || $linha['status'] == "CONSULTA") ) ) {?>
...
<input type="hidden" name="esn">

<span class="line-esn">
<select id="select-esn" name="select-esn" onChange="checkcontratos(this.value)">
<option value="<?php echo $linha['esn']; ?>" selected="selected"><?php echo $linha['esn'];?></option>
</select>
</span>
...
<? } ?>
	...
	<?php
		...
		$querRep = "Select * from reposicoes_clarofixo where id_venda='" . $linha['id'] . "' order by id desc";
		
		$reposicoes = $conexao->query($querRep);
		
		if (mysql_num_rows($reposicoes) > 0 ){
			...
			while ($reposicao = mysql_fetch_assoc($reposicoes)){
				$data = date('d/m/Y',strtotime($reposicao['data']));
				echo "<div style=\"font-size:12px;\">\"" . $reposicao['esn_antiga']  . "\" reposta por \"" . $reposicao['esn_nova'] . "\" em " . $data . "</div>";
			}
...
<tr <?php if( $linha['status']!="FINALIZADA" || (($USUARIO['tipo_usuario']!='ADMINISTRADOR') && ($USUARIO['tipo_usuario']!='MONITOR' && $USUARIO['acesso_usuario']!='INTERNO'))){ ?>style="display:none;"<?php } ?>>
...
<td>
		...
		<input id="bt-reposicao" type="button" style="background-image:url(img/bt_ok.png); border:0px; heigth:25px; cursor:pointer; width:25px;">
...
<? if( ($editar == '1' || $editar_instalacao == '1') && ( (($USUARIO['tipo_usuario']!="MONITOR" && $USUARIO['acesso_usuario']!="EXTERNO") || $linha['status'] == "CONSULTA") ) ) {?>
...
<? if( ($editar == '1') && ( (($USUARIO['tipo_usuario']!="MONITOR" && $USUARIO['acesso_usuario']!="EXTERNO") || $linha['status'] == "CONSULTA")) ) {?>
...
<? if(
($editar == '1' && ( (($USUARIO['tipo_usuario']!="MONITOR" && $USUARIO['acesso_usuario']!="EXTERNO") || $linha['status'] == "CONSULTA") )) /*||
...
if( ($editar == '1' && ($USUARIO['tipo_usuario'] == 'ADMINISTRADOR' || $USUARIO['id']==3129)) && ( (($USUARIO['tipo_usuario']!="MONITOR" && $USUARIO['acesso_usuario']!="EXTERNO") || $linha['status'] == "CONSULTA") ) ) {
...
<td> <? if( ($editar == '1') && ( (($USUARIO['tipo_usuario']!="MONITOR" && $USUARIO['acesso_usuario']!="EXTERNO") || $linha['status'] == "CONSULTA") ) ) {?><span style="font-size:12px; color:#999; font-style:italic">R$</span> <input type="text" id="valorplano" name="valorplano" readonly size="8" maxlength="10"  value="<?= $linha['valorPlano']; ?>" />
...
<td> <? if( ($editar == '1') && ( (($USUARIO['tipo_usuario']!="MONITOR" && $USUARIO['acesso_usuario']!="EXTERNO") || $linha['status'] == "CONSULTA") ) ) {?> <span style="font-size:12px; color:#999; font-style:italic">R$</span> <input type="text" id="valoraparelho" name="valoraparelho" readonly size="8" maxlength="10" value="<?= $linha['valorAparelho']; ?>" />
...
<? if(($editar == '1' && $USUARIO['tipo_usuario'] == 'ADMINISTRADOR') && ( (($USUARIO['tipo_usuario']!="MONITOR" && $USUARIO['acesso_usuario']!="EXTERNO") || $linha['status'] == "CONSULTA") ) ) {?>
...
$currentStatusQualidade = $statusQualidades['atual'];

unset($statusQualidades['atual']);
unset($statusQualidades[0]);

ksort($statusQualidades);

?>

<tr>
	<td><b>Status Portal:</b></td>
	<td><?php echo $saidaTexto->clear($currentStatusQualidade["status_portal"]); ?></td>
</tr>

<tr>
	<td><b>Status Xerox:</b></td>
	<td><?php echo $saidaTexto->clear($currentStatusQualidade["status_xerox"]); ?></td>
</tr>
...
foreach($statusQualidades as $i=>$val)
{
	
	switch ($statusQualidades[$i]['status_portal']){
		
		case 'INTENCIONADA':
		
			$stpLabel = "Data Intenção";
			break;
		
		case 'VENDA CONFIRMADA':
		
			$stpLabel = "Data do Primeiro pcc";
			break;
		
		case 'ATIVADA':
		
			$stpLabel = "Data Ativação";
			break;
		...
		case 'COMISSIONADA':
			...
			if($statusQualidades[$i]['status_data']!='-'){
			
				$statusQualidades[$i]['status_data'] = substr($statusQualidades[$i]['status_data'],3);
	...
	echo "<tr><td style=\"font-size:12px\"><b>" . $stpLabel . ":</b></td><td>" . $statusQualidades[$i]['status_data'] . "</td></tr>"; 
