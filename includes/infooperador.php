...
$conINFOOPERADOR = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && operador = '".$operador."' && (data LIKE '%".$ano.$mes."%' || data_instalacao LIKE '%".$ano.$mes."%' || data_conectada LIKE '%".$ano.$mes."%')  ORDER BY status ASC");
...
$conINFOOPERADORCONECTADO = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && operador = '".$operador."' && data_conectada LIKE '%".$ano.$mes."%' && status = 'CONECTADO'");
...
$numINFOOPERADORREST = mysql_num_rows($conINFOOPERADORREST);$numINFOOPERADORCONECTADO = mysql_num_rows($conINFOOPERADORCONECTADO);
...
echo '<b style="font-size:14px;">VENDAS: '.$numINFOOPERADOR.'</b> &nbsp; &nbsp; <b>Finalizadas:</b> '.$numINFOOPERADORCONEC.' &nbsp; <b>Conectadas:</b> '.$numINFOOPERADORCONECTADO.'. &nbsp; <b>Restrições:</b> '.$numINFOOPERADORREST.' &nbsp; <b>Canceladas:</b> '.$numINFOOPERADORCANC; } ?></span></span>
