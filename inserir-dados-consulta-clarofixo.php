...
//if($cpf == '' || $cpf == '000.000.000-00' || $cpf == '111.111.111-11' || $cpf == '00.000.000/0000-00'){  = "GRAVAR"; } else { $status = "PRE-ANALISE";}
...
$novaVendaId = mysql_insert_id();
//LOG

$datadehoje = date("Y-m-d H:i:s");

$insert_log = $conexao->query("INSERT into log_sistema (data,usuario,evento) VALUES ('".$datadehoje."','".$_SESSION['usuario']."','Inseriu um nova consulta no sistema " . $novaVendaId . ".')");
...
if( isset($_SESSION['usuario'])){
	...
	if( $_POST['status'] == 'nova-venda' ){
		...
		echo "window.location = '?p=inserir-dados-consulta-clarofixo';";
	}
...
$(document).ready(function() {
	...
	$("input[name='status']").bind('click', function(){
		...
		if ( $(this).val() != 'RESTRIÇÃO' ){
			
			$("#tr-os").css("display", "table-row");
			$("#os").focus();
		
		}else{
			
			$("#tr-os").css("display", "none");
			$("#os").val("");
		}
	});
...
if(document.getElementById('idcpf').value == '' ){ document.getElementById('ecpf').style.display = ''; e=(e+1)} else { document.getElementById('ecpf').style.display = 'none';}
...
if(status == '' || typeof status === 'undefined' ){ document.getElementById('estatus').style.display = ''; e=(e+1)} else { document.getElementById('estatus').style.display = 'none';}

if(status!='RESTRIÇÃO' && $("#os").val() == '') { document.getElementById('eos').style.display = ''; e=(e+1)} else { document.getElementById('eos').style.display = 'none';}
...
<td style="font-size:14px; color:#999;">NOVA CONSULTA</td>
...
<td>Status Consulta:</td>

<td>

<input type="radio" id="status1" name="status" value="CONSULTA" /> Aprovada

<input type="radio" id="status2" name="status" value="RESTRIÇÃO" /> Restrição

<input type="radio" id="status3" name="status" value="nova-venda" /> Nova Venda

<span class="campoobrigatorio" title="Campo Obrigatório">*</span>

<span class="erro" id="estatus" style="display:none">Por favor, seleciono status da consulta!</span>
...
<tr id="tr-os" align="left" style="display:none;">
...
<span class="erro" id="eos" style="display:none">Por favor, digite o número da os gerada!</span>
