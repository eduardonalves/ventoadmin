...
if(($USUARIO['tipo_usuario'] == 'MONITOR') || ($USUARIO['tipo_usuario'] == 'SUPERVISOR')){
	$con1 = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '".$idPRODUTO."' && data_autorizacao LIKE '%".$ano.$mes.$dia."%' && status = 'AUTORIZADA' && ($loginMONITOR)");
	$total1 = mysql_num_rows($con1);

	$con2 = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '".$idPRODUTO."' && data_autorizacao LIKE '%".$ano.$mes.$dia."%' && status = 'PÓS VENDAS' && ($loginMONITOR)");
	$total2  = mysql_num_rows($con2);

	$con3 = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '".$idPRODUTO."' && data_autorizacao LIKE '%".$ano.$mes.$dia."%' && status = 'ATIVADO' && ($loginMONITOR)");
	$total3 = mysql_num_rows($con3);
}else{
	$con1 = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '".$idPRODUTO."' && data_autorizacao LIKE '%".$ano.$mes.$dia."%' && status = 'AUTORIZADA'");
	$total1 = mysql_num_rows($con1);

	$con2 = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '".$idPRODUTO."' && data_autorizacao LIKE '%".$ano.$mes.$dia."%' && status = 'PÓS VENDAS'");
	$total2  = mysql_num_rows($con2);

	$con3 = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '".$idPRODUTO."' && data_autorizacao LIKE '%".$ano.$mes.$dia."%' && status = 'ATIVADO'");
	$total3 = mysql_num_rows($con3);
...
<html>
    ...
    <body>
        ...
        <div id="legenda2">
        <span class="qdr1G"></span> <span class="leg1G">Autoriz.<span style="font-size:10px">(<?= $total1;?>)</span></span> 
        <span class="qdr2G"></span> <span class="leg2G">Pós Ven.<span style="font-size:10px">(<?= $total2;?>)</span></span>
        <span class="qdr3G"></span> <span class="leg3G">Ativado <span style="font-size:10px">(<?= $total3;?>)</span></span>
        </div>
    </body>
