...
$con1 = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '4' && data LIKE '%".$ano.$mes.$dia."%' && (status = 'DEVOLVIDO') && monitor LIKE '%".$loginMONITOR."%'");
$total1 = mysql_num_rows($con1);

$con2 = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '4' && data LIKE '%".$ano.$mes.$dia."%' && (status = 'CANCELADO') && monitor LIKE '%".$loginMONITOR."%'");
$total2  = mysql_num_rows($con2);

$con3 = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '4' && data LIKE '%".$ano.$mes.$dia."%' && (status = 'SEM CONTATO') && monitor LIKE '%".$loginMONITOR."%'");
...
<html>
    ...
    <body>
        ...
        <div id="legenda">
        	<a href="../../adm?ve=1&me=<?= $mes;?>&an=<?= $ano;?>&p=oi&pro=4&s=DEVOLVIDO" target="_top">
            	<span class="qdr1" title="Visualizar vendas no status Devolvido"></span>
            </a> 
            <span class="leg1" >Devolvido <span style="font-size:8px">(<?= $total1;?>)</span></span> 
            
            <a href="../../adm?ve=1&me=<?= $mes;?>&an=<?= $ano;?>&p=oi&pro=4&s=CANCELADO" target="_top">
            	<span class="qdr2" title="Visualizar vendas no status Cancelado"></span>
            </a> 
            <span class="leg2">Cancelado <span style="font-size:8px">(<?= $total2;?>)</span></span>
            
            <a href="../../adm?ve=1&me=<?= $mes;?>&an=<?= $ano;?>&p=oi&pro=4&s=SEM+CONTATO" target="_top">
            	<span class="qdr3" title="Visualizar vendas no status Sem Contato"></span>
            </a> 
            <span class="leg3">Sem Contato <span style="font-size:8px">(<?= $total3;?>)</span></span>
