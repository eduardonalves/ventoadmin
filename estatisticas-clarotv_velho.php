...
<td bgcolor="#77c8f0"> &nbsp; Percentual de Instalações &nbsp; </td>
...
$conINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && data_instalacao LIKE '%".$ano.$mes."%' && status = 'CONECTADO' && monitor LIKE '%".$loginMONITOR."%'");
...
$conNUMINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && data_instalacao = '".$ano.$mes.$n."' && status = 'CONECTADO' && monitor LIKE '%".$loginMONITOR."%'");
...
$conMR = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tecnico_id = '1' && data_instalacao LIKE '%".$ano.$mes."%'");

$totalMR = mysql_num_rows($conMR);



$conPOADMR = $conexao->query("SELECT sum(pontos) AS soma FROM vendas_clarotv WHERE produto = '".$produto_id."' && tecnico_id = '1' && data_instalacao LIKE '%".$ano.$mes."%'");
...
$conINFOTEC = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tecnico_id = '1' && status = 'CONECTADO' && data_instalacao LIKE '%".$ano.$mes."%' ORDER BY data_instalacao ASC");
...
$conTEC = $conexao->query("SELECT * FROM tecnicos WHERE tecnico_id != '1' ORDER BY nome");
...
$conINSTTEC = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tecnico_id = '".$TEC['tecnico_id']."' && data_instalacao LIKE '%".$ano.$mes."%'");

$totalINSTTEC = mysql_num_rows($conINSTTEC);



$conPOAD = $conexao->query("SELECT sum(pontos) AS soma FROM vendas_clarotv WHERE produto = '".$produto_id."' && tecnico_id = '".$TEC['tecnico_id']."' && data_instalacao LIKE '%".$ano.$mes."%' && status = 'CONECTADO'");
...
$conINFOTEC = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tecnico_id = '".$TEC['tecnico_id']."' && status = 'CONECTADO' && data_instalacao LIKE '%".$ano.$mes."%' ORDER BY data_instalacao ASC");
...
$conVENDASCONECTADAS = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && operador = '".$OPERADORES['operador_id']."' && monitor = '".$MONITORES['id']."' && data_instalacao LIKE '%".$ano.$mes."%' && status = 'CONECTADO'");
...
<? if($OPERADORES['status'] == 'DESLIGADO') {?><span style="color:#A9A9A9;"><?= $OPERADORES['nome'];?></span> <? } else { echo $OPERADORES['nome']; } ?>
...
$conINFOOPERADORCONEC = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && operador = '".$OPERADORES['operador_id']."' && data_instalacao LIKE '%".$ano.$mes."%'");



$conINFOOPERADORREST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && operador = '".$OPERADORES['operador_id']."' && data LIKE '%".$ano.$mes."%' && status = 'RESTRIÇÃO'");
...
echo '<b style="font-size:14px;">VENDAS: '.$numINFOOPERADOR.'</b> &nbsp; &nbsp; <b>Conectadas:</b> '.$numINFOOPERADORCONEC.' &nbsp; <b>Restrições:</b> '.$numINFOOPERADORREST.' &nbsp; <b>Canceladas:</b> '.$numINFOOPERADORCANC; } ?></span></span>
...
<td width="100px">Data Instalada</td>
...
$conVENDASCONECTADASM = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && monitor = '".$MONITORES['id']."' && data_instalacao LIKE '%".$ano.$mes."%' && status = 'CONECTADO'");
...
<table border="0" width="90%" id="anyid">
...
<td title="Vendas Conectadas" onclick="window.location = '?p=estatisticas-clarotv&m=<?= $_GET['m'];?>&an=<?= $_GET['an'];?>&es=<?= $_GET['es'];?>&o=<? if($_GET['o'] == 'conectadas DESC' || $_GET['o'] == ''){ echo 'conectadas ASC'; } else { echo 'conectadas DESC'; }?>'">Vendas Conectadas <? if($_GET['o'] == 'conectadas DESC' || $_GET['o'] == ''){ ?><img src="img/seta-d.png" /> <? } else if($_GET['o'] == 'conectadas ASC'){ ?> <img src="img/seta-u.png" /> <? } ?></td>
...
$conOPERADORES1 = $conexao->query("SELECT *, COUNT(IF(vendas_clarotv.data LIKE '%".$ano.$mes."%', 1, NULL)) AS vendas,
									 ...
									 ((COUNT(IF(vendas_clarotv.status = 'CONECTADO' && 
									 		  ...
									 		  vendas_clarotv.data_instalacao LIKE '%".$ano.$mes."%', 1, NULL)) * 100) / COUNT(IF(vendas_clarotv.data LIKE '%".$ano.$mes."%', 1, NULL))) AS aproveitamento,
...
<td width="100px"><?= strtoupper($INFOOPERADOR['plano']);?></td>

<td width="120px"><?= strtoupper($INFOOPERADOR['status']);?></td>
...
$conVENDASCONECTADASM = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && tipoVenda = 'INTERNA' && data_instalacao LIKE '%".$ano.$mes."%' && status = 'CONECTADO'");
...
<tr align="center" class="sortbottom" style="font-size:10px; font-weight:bold;" bgcolor="#CCCCCC">
