...
$conINFOOPERADOR = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && operador = '".$operador."' && (data LIKE '%".$ano.$mes."%' || data_autorizacao LIKE '%".$ano.$mes."%')  ORDER BY status ASC");
...
$conINFOOPERADORCONEC = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$produto_id."' && operador = '".$operador."' && data_autorizacao LIKE '%".$ano.$mes."%' && (status = 'AUTORIZADA' || status = 'PÓS VENDAS')");
...
echo '<b style="font-size:14px;">VENDAS: '.$numINFOOPERADOR.'</b> &nbsp; &nbsp; <b>Finalizadas:</b> '.$numINFOOPERADORCONEC.' &nbsp; <b>Devolvidas:</b> '.$numINFOOPERADORCONECTADO.'. &nbsp; <b>Restrições:</b> '.$numINFOOPERADORREST.' &nbsp; <b>Canceladas:</b> '.$numINFOOPERADORCANC; } ?></span></span>
