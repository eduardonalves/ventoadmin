...
$conGRAVAR = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '".$idPRODUTO."' && data LIKE '%".$ano.$mes.$dia."%' && status = 'GRAVAR' && monitor LIKE '%".$loginMONITOR."%'");
$totalGRAVAR = mysql_num_rows($conGRAVAR);

$conAPROVADO = $conexao->query("SELECT * FROM vendas_clarotv  WHERE produto = '".$idPRODUTO."' && data LIKE '%".$ano.$mes.$dia."%' && status = 'APROVADO' && monitor LIKE '%".$loginMONITOR."%'");
