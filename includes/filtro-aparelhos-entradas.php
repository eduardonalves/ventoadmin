...
$query = "SELECT entradas.id_entrada, entradas.data, entradas.nf, entradas.id_estoquista, entradas.origem, userEstoq.nome AS estoquista, DATE_FORMAT(entradas.data, '%d/%m/%Y &agrave;s %H:%i:%s') AS data
				   				   FROM entradas 
				                   INNER JOIN usuarios userEstoq ON (userEstoq.id=entradas.id_estoquista)
				                   WHERE 1=1
								   ";

if($_GET["est"]!="") { $query.= " && userEstoq.nome= \"" . $_GET["est"] . "\""; }
if($_GET["nf"]!="") { $query.= " && (entradas.nf= \"" . $_GET["nf"] . "\")"; }
...
$query.= " && entradas.data BETWEEN '" . $dataInicio . "' AND '" . $dataFinal . "'";
$order = $_GET["o"];
...
if($order!="")
{
	...
	$fOrder = str_replace("nota", "entradas.nf", $fOrder);
	$fOrder = str_replace("data", "data", $fOrder);
	$fOrder = str_replace("origem", "entradas.origem", $fOrder);
	
	$query .= " ORDER BY " . $fOrder;
}

$conEntradas = $conexao->query($query . " LIMIT $inicial, $numreg");
