...
<td title="Reposições">Reposições</td>
...
foreach($monitores as $monitor)
{
	...
	$total_monitor_reposicao = $estoque->getQuantTotalParceiroReposicao($monitor["id"], $g_dataInicio, $g_dataFinal);
	?>
<tr>
	...
	<td style="background-color:#999999; text-align:center;color:#FFF; font-size:12px; font-weight:bold;"><?=$total_monitor_reposicao?></td>
...
<?
	...
	foreach($itensToFilter as $ln)
	{
		...
		foreach($itens as $item)
		{
			...
			{	
				...
				$query = "Select count(id) as qt from reposicoes_clarofixo
				WHERE (monitor_venda='" . $monitor['id'] . "') && id_aparelho=". $line["id_aparelho"] . " && data BETWEEN '$g_dataInicio' AND '$g_dataFinal'";
			...
			$itensAgrupados[$cont-1]["ap_reposicoes"] += $qtotalReposicao;
		}// each itens
	...
	?>
			...
			<td title="Quantidade Reposicoes" <? if(strstr($_GET['o'],'origem')){ ?>class="tdselected" <? } ?>><?= $linhas["ap_reposicoes"];?></td>
