...
if(document.getElementById('tipoBandaLarga').value == ''){ document.getElementById('etipoBandaLarga').style.display = ''; e=(e+1)} else { document.getElementById('etipoBandaLarga').style.display = 'none';}	
...
{
	$conMONITORES = $conexao->query("SELECT * FROM usuarios WHERE tipo_usuario = 'MONITOR' && grupo LIKE '%0002%' && status='ATIVO' && id='" . $USUARIO["id"] . "' order by nome");
}else{
	
	$conMONITORES = $conexao->query("SELECT * FROM usuarios WHERE tipo_usuario = 'MONITOR' && grupo LIKE '%0002%' && status='ATIVO' order by nome");
}
...
<tr align="left">
	...
	<td>
		...
		<select id="tipoBandaLarga" name="tipoBandaLarga">
			...
			<option value="3GMax">3GMax</option>
			<option value="4GMax">4GMax</option>
		
		</select>
		
		<span class="campoobrigatorio" title="Campo Obrigatório">*</span>

		<span class="erro" id="etipoBandaLarga" style="display:none">Por favor, selecione o tipo de tecnologia!</span>
...
<input type="hidden" name="tipoEntrega" value="INTERNA">
<input type="hidden" name="sistema" value="V-SALES">
