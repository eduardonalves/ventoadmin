...
// Autorizadas (Autorizada + Pós Vendas + Ativado)
$conINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && data_autorizacao LIKE '%".$ano.$mes.$dia."%' && (status = 'AUTORIZADA' || status = 'PÓS VENDAS' || status = 'ATIVADO') && ($loginMONITOR)");
$totalINST = mysql_num_rows($conINST);

// Ativado
$conPEND = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'ATIVADO' && data LIKE '%".$ano.$mes.$dia."%' && ($loginMONITOR)");
...
// Auditoria (Pré-Análise + Pré-aprovado + Gravar + Gravado)
$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'PRE-ANALISE' || status = 'PRE-APROVADO' || status = 'GRAVAR' || status = 'GRAVADO') && data LIKE '%".$ano.$mes.$dia."%' && ($loginMONITOR)");
...
// Autorizadas (Autorizada + Pós Vendas + Ativado)
$conINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && data_autorizacao LIKE '%".$ano.$mes.$dia."%' && (status = 'AUTORIZADA' || status = 'PÓS VENDAS' || status = 'ATIVADO')");
$totalINST = mysql_num_rows($conINST);

// Ativado
$conPEND = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'ATIVADO' && data LIKE '%".$ano.$mes.$dia."%'");
$totalPEND = mysql_num_rows($conPEND);
...
// Auditoria (Pré-Análise + Pré-aprovado + Gravar + Gravado)
$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'PRE-ANALISE' || status = 'PRE-APROVADO' || status = 'GRAVAR' || status = 'GRAVADO') && data LIKE '%".$ano.$mes.$dia."%'");
$totalOUT = mysql_num_rows($conOUT);
}
switch ($mes) {
...
document.getElementById('grafico').innerHTML = '<div id="fecharpacotes" onclick="escondergrafico();">VOLTAR</div><iframe frameborder="0" scrolling="no" id="framepacotes" src="grafico/grafico-claro3g-'+gra+'.php?d=<?= $_GET['d'];?>&m=<?= $_GET['m'];?>&a=<?= $_GET['a'];?>"></iframe>';
...
<div id="chart3" class="chart" onMouseOver="showinfo('i3')" onMouseOut="hideinfo('i3')" onClick="mostrargrafico('ativados');">
