...
$agendEntrega = $_POST['agendEntrega'];

if($agendEntrega != ''){

	$agendEntrega = explode('/',$_POST['agendEntrega']);
	$agendEntrega = $agendEntrega[2].'-'.$agendEntrega[1].'-'.$agendEntrega[0].' 00:00:00';

}else{
	
	$agendEntrega = '0000-00-00 00:00:00';
...
else if($_POST['status'] == 'RECUPERADO' ){ $linha['auditor'] = ''; $linha['gravacao'] = '';  $status = 'PRE-ANALISE'; }
...
<script type="text/javascript">
	...
	function carregaTiposEntregas(sistema){
		
		if(sistema == 'W.A.'){
			
			var opcoes_entrega = "<option value=\"CLARO\" selected=\"selected\">CLARO</option>";
			
		}else if (sistema == 'V-SALES'){
				
			var opcoes_entrega = "<option value=\"INTERNA\" selected=\"selected\">INTERNA</option>" + 
									"<option value=\"MR\">MR</option>";
		}
			
		$('#tipoEntrega').html(opcoes_entrega);
	...
	function carregaCampos(tipoEntrega){
		if ($('#tipoEntrega').length < 1){
		
			var tipoEntrega = "<?php echo $linha['tipoEntrega']; ?>";
			...
			if( tipoEntrega == 'CLARO' ){
				
				$("#tr-contrato").css('display', 'none');
				$("#tr-imei").css('display', 'none');
				$("#tr-simcard").css('display', 'none');
				$("#tr-proposta").css('display', 'none');

				$("#tr-codautorizacao").css('display', 'table-row');
				$("#tr-numordem").css('display', 'table-row');
				$("#tr-msisdn").css('display', 'table-row');

				$("#tr-agendentrega").css('display', 'none');
				$("#tr-agendentrega input").val('');
								
			}else if( tipoEntrega == 'INTERNA' ){

				$("#tr-contrato").css('display', 'table-row');
				$("#tr-imei").css('display', 'table-row');
				$("#tr-simcard").css('display', 'table-row');
				$("#tr-proposta").css('display', 'table-row');

				$("#tr-codautorizacao").css('display', 'none');
				$("#tr-numordem").css('display', 'none');
				$("#tr-msisdn").css('display', 'none');
				
				$("#tr-agendentrega").css('display', 'none');
				$("#tr-agendentrega input").val('');

			}else if (tipoEntrega == 'MR'){

				$("#tr-agendentrega").css('display', 'table-row');
	...
	$(window).load( function(){
		...
		carregaTiposEntregas("<?php echo $linha['tipoEntrega']; ?>");
	...
	$(document).ready( function(){
		...
		$("#sistema").change( function() {
			...
			var sistema = $('option:selected', this).val();		
			
			carregaTiposEntregas(sistema);
...
document.getElementById('mdevolvido').style.display = 'none';  
...
document.getElementById('mcancel').style.display = 'none'; 
document.getElementById('mdevolvido').style.display = '';  
document.getElementById('mrest').style.display = 'none';
...
document.getElementById('motivo_devolvido').value = "";
...
if(v == "12GB"){ document.getElementById('valor').value = '129,90'; }

else if(v == "10GB"){ document.getElementById('valor').value = '129,90'; }

else if(v == "6GB"){ document.getElementById('valor').value = '71,90'; }

else if(v == "5GB"){ document.getElementById('valor').value = '95,90'; }

else if(v == "4GB"){ document.getElementById('valor').value = '64,90'; }
...
var tecnologia = $("[name='tipoBandaLarga'] option:selected").val();
var tipoEntrega = $("[name='tipoEntrega'] option:selected").val();
...
if(tecnologia==''){
	...
	alert("Selecione a tecnologia de banda larga.");
	erro = erro+1;
...
}else if (tipoEntrega=='MR' && $("[name='status']").val() == 'ENTREGAR'){
	
	if( $('#agendEntrega').val().length < 10 ){
	
		alert("Digite corretamente a data da entrega.");
		erro = erro+1;
...
<tr id="tr-contrato" style="display:none;">
...
<input type="text" name="contrato" size="40" maxlength="11" value="<?= $linha['contrato']; ?>" />
...
<tr id="tr-proposta" style="display:none;">
...
<input type="text" name="proposta" size="40" maxlength="11" value="<?= $linha['proposta']; ?>" />
...
<tr id="tr-msisdn" style="display:none;">
...
<tr id="tr-codautorizacao" style="display:none;">
...
<tr id="tr-numordem" style="display:none;">
...
<tr id="tr-imei" style="display:none;">
...
<input type="text" name="imei" size="40" maxlength="15" value="<?= $linha['imei']; ?>" />

<? } else { ?>

<?= $linha['imei']; ?>

<input type="hidden" name="imei" size="40" value="<?= $linha['imei']; ?>" />
...
<tr id="tr-simcard" style="display:none;">
...
<input type="text" name="simcard" size="40" maxlength="20" value="<?= $linha['simcard']; ?>" />

<? } else { ?>

<?= $linha['simcard']; ?>

<input type="hidden" name="simcard" size="40" value="<?= $linha['simcard']; ?>" />
...
<? if($editar == '1') {?>
		...
		<select id="tipoBandaLarga" name="tipoBandaLarga">
			
			<option value="">Selecione...</option>
			<option value="3GMax" <? if($linha['tipoBandaLarga'] == '3GMax'){ ?> selected="selected" <? }?>>3GMax</option>
			<option value="4GMax" <? if($linha['tipoBandaLarga'] == '4GMax'){ ?> selected="selected" <? }?>>4GMax</option>
...
<?= $linha['tipoBandaLarga']; ?>



<input type="hidden" name="tipoBandaLarga" value="<?= $linha['tipoBandaLarga']; ?>" />
...
<option value="12GB" <? if($linha['plano'] == '12GB'){ ?> selected="selected" <? }?>>12GB</option>

<? if($linha['plano'] == '10GB'){ ?><option value="10GB" selected="selected">10GB</option><? }?>

<option value="6GB" <? if($linha['plano'] == '6GB'){ ?> selected="selected" <? }?>>6GB</option>

<option value="4GB" <? if($linha['plano'] == '4GB'){ ?> selected="selected" <? }?>>4GB</option>

<? if($linha['plano'] == '2GB'){ ?><option value="2GB" selected="selected">2GB</option> <? }?>
<? if($linha['plano'] == '1GB'){ ?><option value="1GB" selected="selected">1GB</option><? }?>
...
<option value="W.A." <? if($linha['sistema'] == 'W.A.'){?>selected="selected"<? } ?>>W.A.</option>

<option value="V-SALES" <? if($linha['sistema'] == 'V-SALES' || $linha['sistema'] == ''){?>selected="selected"<? } ?>>V-SALES</option>
...
<input type="hidden" value="<?= $linha['sistema'];?>" name="sistema" />
...
if($linha['sistema']=='V-SALES' || $linha['sistema']==''){
?>	

<option value="INTERNA" <? if($linha['tipoEntrega'] == 'INTERNA'){?>selected="selected"<? } ?>>Interna</option>

<option value="MR" <? if($linha['tipoEntrega'] == 'MR'){?>selected="selected"<? } ?>>MR</option>

<?php
}elseif ($linha['sistema']=='W.A.'){?>
?>

<option value="CLARO" <? if($linha['tipoEntrega'] == 'CLARO'){?>selected="selected"<? } ?>>Claro</option>
<?php
}
?>
</select>
...
<input type="hidden" value="<?= $linha['tipoEntrega'];?>" name="tipoEntrega" />
...
$agendEntrega = strtotime($linha['agendEntrega']);
...
<input type="text" id="agendEntrega" onkeypress="mascara(this,data)" onkeyup="validadata(this.value,agendagravacao)" maxlength="10" value="<?= $agendEntrega;?>" name="agendEntrega" />
...
<input type="hidden" value="<?= $linha['agendEntrega'];?>" name="agendEntrega" />
...
<?php if ($USUARIO['tipo_usuario'] == 'MONITOR' || $USUARIO['tipo_usuario'] == 'ADMINISTRADOR' )

{
	?>
		...
		<? if($linha['motivo_cancelamento'] == 'Inviabilidade Técnica'){?><option value="Inviabilidade Técnica" selected="selected">Inviabilidade Técnica</option><? } ?>

		<? if($linha['motivo_cancelamento'] == 'Falta de Dinheiro'){?><option value="Falta de Dinheiro" selected="selected">Falta de Dinheiro</option><? } ?>

		<? if($linha['motivo_cancelamento'] == 'Venda Perdida para a Concorrência'){?><option value="Venda Perdida para a Concorrência" selected="selected">Venda Perdida para a Concorrência</option><? } ?>

		<? if($linha['motivo_cancelamento'] == 'Desistência do Cliente'){?><option value="Desistência do Cliente" selected="selected">Desistência do Cliente</option><? } ?>

		<? if($linha['motivo_cancelamento'] == 'Endereço Não Encontrado'){?><option value="Endereço Não Encontrado" selected="selected">Endereço Não Encontrado</option><? } ?>

		<? if($linha['motivo_cancelamento'] == 'Área de Risco'){?><option value="Área de Risco" selected="selected">Área de Risco</option><? } ?>

		<? if($linha['motivo_cancelamento'] == 'Cancelado no VSALES'){?><option value="Cancelado no VSALES" selected="selected">Cancelado no VSALES</option><? } ?>

		<option value="CQ- SUSPEITA DE FRAUDE-PRÉ ANALISE" <? if($linha['motivo_cancelamento'] == 'CQ- SUSPEITA DE FRAUDE-PRÉ ANALISE'){?>selected="selected"<? } ?>>CQ- SUSPEITA DE FRAUDE-PRÉ ANALISE</option>

		<option value="CL DESISTIU - MOTIVOS FINANCEIROS" <? if($linha['motivo_cancelamento'] == 'CL DESISTIU - MOTIVOS FINANCEIROS'){?>selected="selected"<? } ?>>CL DESISTIU - MOTIVOS FINANCEIROS</option>

		<option value="CQ - REPROVADO ÁREA SEM COBERTURA" <? if($linha['motivo_cancelamento'] == 'CQ - REPROVADO ÁREA SEM COBERTURA'){?>selected="selected"<? } ?>>CQ - REPROVADO ÁREA SEM COBERTURA</option>

		<option value="REPROVADO AUDITORIA" <? if($linha['motivo_cancelamento'] == 'REPROVADO AUDITORIA'){?>selected="selected"<? } ?>>REPROVADO AUDITORIA</option>

		<option value="CANCELADO" <? if($linha['motivo_cancelamento'] == 'CANCELADO'){?>selected="selected"<? } ?>>CANCELADO</option>

		<option value="INFORMAÇÃO DIVERGENTE" <? if($linha['motivo_cancelamento'] == 'INFORMAÇÃO DIVERGENTE'){?>selected="selected"<? } ?>>INFORMAÇÃO DIVERGENTE</option>
	...
	<?php }else{ ?>
		...
		<option value="<?=$linha['motivo_cancelamento'];?>" selected="selected"><?=$linha['motivo_cancelamento'];?></option>
...
<? if($linha['motivo_restricao'] == 'Fraude'){?><option value="Fraude" selected="selected">Fraude</option><? } ?>
<? if($linha['motivo_restricao'] == 'Consultas Excedidas'){?><option value="Consultas Excedidas" selected="selected">Consultas Excedidas</option><? } ?>
...
<option value="Débitos com a claro" <? if($linha['motivo_restricao'] == 'Débitos com a claro'){?>selected="selected"<? } ?>>Débitos com a claro</option>
<option value="Limite de linhas ( limite excedido)" <? if($linha['motivo_restricao'] == 'Limite de linhas ( limite excedido)'){?>selected="selected"<? } ?>>Limite de linhas ( limite excedido)</option>
<option value="Limite de credito não liberado" <? if($linha['motivo_restricao'] == 'Limite de credito não liberado'){?>selected="selected"<? } ?>>Limite de credito não liberado</option>
<option value="Menor de 18 anos" <? if($linha['motivo_restricao'] == 'Menor de 18 anos'){?>selected="selected"<? } ?>>Menor de 18 anos</option>
<option value="Análise prévia" <? if($linha['motivo_restricao'] == 'Análise prévia'){?>selected="selected"<? } ?>>Análise prévia</option>
<option value="Dados divergentes" <? if($linha['motivo_restricao'] == 'Dados divergentes'){?>selected="selected"<? } ?>>Dados divergentes</option>
<option value="CPF em avaliação jurídica" <? if($linha['motivo_restricao'] == 'CPF em avaliação jurídica'){?>selected="selected"<? } ?>>CPF em avaliação jurídica</option>
...
<tr id="mdevolvido" <? if($linha['status'] != 'DEVOLVIDO'){ ?> style="display:none" <? } ?>>
...
<select name="motivo_devolvido" id="motivo_devolvido">

<option value=""></option>

	<option value="CPF Inválido" <? if($linha['motivo_devolvido'] == 'CPF Inválido'){?>selected="selected"<? } ?>>CPF Inválido</option>
