...
$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'PRE-ANALISE' || status = 'ANÁLISE' || status = 'GRAVAR' || status = 'APROVADO') && data LIKE '%".$ano.$mes.$dia."%' && ($loginMONITOR)");
...
//Auditoria (Pré-Análise + Análise + Gravar + Aprovado)
$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'PRE-ANALISE' || status = 'ANÁLISE' || status = 'GRAVAR' || status = 'APROVADO') && data LIKE '%".$ano.$mes.$dia."%' ");
$totalOUT = mysql_num_rows($conOUT);
}
