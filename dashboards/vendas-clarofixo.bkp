...
$conVENDAS = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && data LIKE '%".$ano.$mes."%' && monitor LIKE '%".$loginMONITOR."%'");
$totalVENDAS = mysql_num_rows($conVENDAS);


$conINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && data_instalacao LIKE '%".$ano.$mes."%' && status = 'FINALIZADA' && monitor LIKE '%".$loginMONITOR."%'");
$totalINST = mysql_num_rows($conINST);

$conPEND = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'REDIRECIONADO' && data LIKE '%".$ano.$mes."%' && monitor LIKE '%".$loginMONITOR."%'");
$totalPEND = mysql_num_rows($conPEND);

$conRES = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'RESTRIÇÃO' && data LIKE '%".$ano.$mes."%' && monitor LIKE '%".$loginMONITOR."%'");
$totalRES = mysql_num_rows($conRES);

$conCANC = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'CANCELADO' && data LIKE '%".$ano.$mes."%' && monitor LIKE '%".$loginMONITOR."%'");
$totalCANC = mysql_num_rows($conCANC);

//$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && ((status = 'FINALIZADA' && data_instalacao = '') || status = 'BOLETO GERADO' || status = 'DEVOLVIDO' || status = 'ENVIAR GRAVAÇÃO' || status = 'GRAVAR' || status = 'GRAVADO' || status = 'PRE-ANALISE' || status = 'SEM COBERTUTA' || status = 'SEM CONTATO') && data LIKE '%".$ano.$mes."%' && monitor LIKE '%".$loginMONITOR."%'");
$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && ((status = 'FINALIZADA' && data_instalacao NOT LIKE '%".$ano.$mes."%') || (status != 'FINALIZADA' && status != 'REDIRECIONADO' && status != 'RESTRIÇÃO' && status != 'CANCELADO')) && data LIKE '%".$ano.$mes."%' && monitor LIKE '%".$loginMONITOR."%'");
...
document.getElementById('grafico').innerHTML = '<div id="fecharpacotes" onclick="escondergrafico();">VOLTAR</div><iframe frameborder="0" scrolling="no" id="framepacotes" src="grafico/grafico-clarofixo-'+gra+'.php?m=<?= $_GET['m'];?>&a=<?= $_GET['a'];?>"></iframe>';
...
#content, #meses, #anos, #grafico{position:absolute; width:<?= $w;?>px; height:<?= $h;?>px; cursor:default; background: #feffff; /* Old browsers */
...
#meses, #anos{position:absolute; padding:15px; width: 320px; top:0px; background-color:#FFF; z-index:100;}
...
.mes:hover, .ano:hover{ cursor: pointer;
...
<div class="mes" onClick="window.location = '?m=01&a=<?= $_GET['a']?>'">
Janeiro
</div>
<div class="mes" onClick="window.location = '?m=02&a=<?= $_GET['a']?>'">
Fevereiro
</div>
<div class="mes" onClick="window.location = '?m=03&a=<?= $_GET['a']?>'">
Março
</div>
<div class="mes" onClick="window.location = '?m=04&a=<?= $_GET['a']?>'">
Abril
</div>
<div class="mes" onClick="window.location = '?m=05&a=<?= $_GET['a']?>'">
Maio
</div>
<div class="mes" onClick="window.location = '?m=06&a=<?= $_GET['a']?>'">
Junho
</div>
<div class="mes" onClick="window.location = '?m=07&a=<?= $_GET['a']?>'">
Julho
</div>
<div class="mes" onClick="window.location = '?m=08&a=<?= $_GET['a']?>'">
Agosto
</div>
<div class="mes" onClick="window.location = '?m=09&a=<?= $_GET['a']?>'">
Setembro
</div>
<div class="mes" onClick="window.location = '?m=10&a=<?= $_GET['a']?>'">
Outubro
</div>
<div class="mes" onClick="window.location = '?m=11&a=<?= $_GET['a']?>'">
Novembro
</div>
<div class="mes" onClick="window.location = '?m=12&a=<?= $_GET['a']?>'">
Dezembro
</div>
</div>
...
$a = $as++;
?>
<div class="ano" onClick="window.location = '?m=<?= $_GET['m'];?>&a=<?= $a; ?>'">
<?= $a; ?>
</div>
<? } ?>
</div>
...
<div id="chart4" class="chart" onMouseOver="showinfo('i4')" onMouseOut="hideinfo('i4')"  onClick="javascript:window.open('../adm?ve=1&me=<?= $mes;?>&an=<?= $ano; ?>&p=clarofixo&o=&m=&t=&f=&s=RESTRI%C7%C3O&v=&i=&di=&b=','_top');">
...
<div id="chart5" class="chart" onMouseOver="showinfo('i5')" onMouseOut="hideinfo('i5')" onClick="javascript:window.open('../adm?ve=1&me=<?= $mes; ?>&an=<?= $ano; ?>&p=clarotv&o=&m=&t=&f=&s=CANCELADO&v=&i=&di=&b=','_top');">
