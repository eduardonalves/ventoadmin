...
$set_colunas = $conexao->query("UPDATE usuarios SET colunas_aparelhos = '(".$_POST['chk1'].") (".$_POST['chk2'].") (".$_POST['chk3'].")' WHERE id = '".$USUARIO['id']."'");
...
<td width="50%"><input type="checkbox" name="chk1" <? if(strstr($USUARIO['colunas_aparelhos'],'(marca)')){?>checked="checked"<? } ?> value="marca" />MARCA</td>

<td width="50%"><input type="checkbox" name="chk2" <? if(strstr($USUARIO['colunas_aparelhos'],'(modelo)')){?>checked="checked"<? } ?> value="modelo" />MODELO</td>

</tr>

<tr align="left">

<td width="50%"><input type="checkbox" name="chk3" <? if(strstr($USUARIO['colunas_aparelhos'],'(estoque)')){?>checked="checked"<? } ?> value="estoque" />ESTOQUE</td>
...
$getAparelhos = $conexao->query("Select * from aparelhos");
...
<select name="mc" onchange="javascript:document.forms.filtro.submit();">

<option value=""></option>

<?

while ($marca = mysql_fetch_array($getAparelhos))
{

	echo "<option value=\"" . $marca["marca"] . "\""; if($_GET["mc"]==$marca["marca"]){ echo " selected=\"selected\"";} echo ">". $marca["marca"] ."</option>";
}
...
<select name="mod" onchange="javascript:document.forms.filtro.submit();">

<option value=""></option>

<?
$getAparelhos = $conexao->query("Select * from aparelhos");
while ($modelo = mysql_fetch_array($getAparelhos))
{

	echo "<option value=\"" . $modelo["modelo"] . "\""; if($_GET["mod"]==$modelo["modelo"]){ echo " selected=\"selected\""; } echo ">". $modelo["modelo"] ."</option>";
}
...
<td>
 ...
 | Estoque menor ou igual a:

<select name="mqt" onchange="javascript:document.forms.filtro.submit();">

<option value=""></option>

<?
$getAparelhos = $conexao->query("Select DISTINCT estoque from aparelhos order by estoque ASC");
while ($estoque = mysql_fetch_array($getAparelhos))
{

	echo "<option value=\"" . $estoque["estoque"] . "\""; if($_GET["mqt"]==$estoque["estoque"]){ echo " selected=\"selected\"";} echo ">". $estoque["estoque"] ."</option>";
}
...
<select name="pqt" onchange="javascript:document.forms.filtro.submit();">
...
while ($estoque = mysql_fetch_array($getAparelhos))
{
	...
	echo "<option value=\"" . $estoque["estoque"] . "\""; if($_GET["pqt"]==$estoque["estoque"]){ echo " selected=\"selected\"";} echo ">". $estoque["estoque"] ."</option>";
}
...
include "includes/filtro-aparelhos.php";
...
<b><?= $quantreg; ?></b> <? if($quantreg == 1){?>Produto encontrado<? } else {?> Produtos encontrados <? } ?>
...
<input type="hidden" name="mqt" value="<?= $_GET['mqt'];?>" />
<input type="hidden" name="pqt" value="<?= $_GET['pqt'];?>" />
<input type="hidden" name="es" value="<?= $_GET['es'];?>" />
...
<? if(strstr($USUARIO['colunas_aparelhos'],'(marca)')){?>

<td title="Marca" onclick="window.location = '?p=aparelhos-clarofixo&es=<?= $_GET['es'];?>&m=<?= $_GET['m'];?>&mc=<?= $_GET['mc'];?>&mod=<?= $_GET['mod'];?>&mqt=<?= $_GET['mqt'];?>&pqt=<?= $_GET['pqt'];?>&pg=<?php echo ($pg - 1); ?>&o=<? if($_GET['o'] != 'marca DESC'){ echo 'marca DESC'; } else { echo 'marca ASC'; }?>'">Marca <? if($_GET['o'] == 'marca DESC'){ ?><img src="img/seta-d.png" /> <? } else if($_GET['o'] == 'marca ASC'){ ?> <img src="img/seta-u.png" /> <? } ?></td>

<? } ?>



<? if(strstr($USUARIO['colunas_aparelhos'],'(modelo)')){?>

<td title="Modelo" onclick="window.location = '?p=aparelhos-clarofixo&es=<?= $_GET['es'];?>&m=<?= $_GET['m'];?>&mc=<?= $_GET['mc'];?>&mod=<?= $_GET['mod'];?>&mqt=<?= $_GET['mqt'];?>&pqt=<?= $_GET['pqt'];?>&pg=<?php echo ($pg - 1); ?>&o=<? if($_GET['o'] != 'modelo DESC'){ echo 'modelo DESC'; } else { echo 'modelo ASC'; }?>'">Modelo <? if($_GET['o'] == 'modelo DESC'){ ?><img src="img/seta-d.png" /> <? } else if($_GET['o'] == 'modelo ASC'){ ?> <img src="img/seta-u.png" /> <? } ?></td>

<? } ?>


<? if(strstr($USUARIO['colunas_aparelhos'],'(estoque)')){?>

<td title="Quantidade em Estoque" onclick="window.location = '?p=aparelhos-clarofixo&es=<?= $_GET['es'];?>&m=<?= $_GET['m'];?>&mc=<?= $_GET['mc'];?>&mod=<?= $_GET['mod'];?>&mqt=<?= $_GET['mqt'];?>&pqt=<?= $_GET['pqt'];?>&pg=<?php echo ($pg - 1); ?>&o=<? if($_GET['o'] != 'estoque DESC'){ echo 'estoque DESC'; } else { echo 'estoque ASC'; }?>'">Estoque <? if($_GET['o'] == 'estoque DESC'){ ?><img src="img/seta-d.png" /> <? } else if($_GET['o'] == 'estoque ASC'){ ?> <img src="img/seta-u.png" /> <? } ?></td>
...
while($aparelho = mysql_fetch_assoc($conAparelhos)){
...
<td title="Marca" <? if(strstr($_GET['o'],'marca')){ ?> class="tdselected" <? } ?>><?= $aparelho['marca'];?></td>
...
<td title="Modelo" <? if(strstr($_GET['o'],'modelo')){ ?>class="tdselected" <? } ?>><?= $aparelho['modelo'];?></td>
...
<td title="Quantidade em Estoque" <? if(strstr($_GET['o'],'estoque')){ ?>class="tdselected" <? } ?>><?= $aparelho['estoque'];?></td>
